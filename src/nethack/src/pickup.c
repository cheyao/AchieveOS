        ...
        int what; /* should be a long */
{
    ...
    if (flags.menu_style != MENU_TRADITIONAL || iflags.menu_requested) {
        ...
        end_query:; /* statement required after label */
    }
...
for (
n = 0, last = 0, curr = olist;
curr;
curr = FOLLOW(curr, qflags)
)
if ((*allow)(curr)) {
last = curr;
n++;
...
any.
a_obj = curr;
add_menu(win, obj_to_glyph(curr, rn2_on_display_rng),
&any,
...
add_menu(win, NO_GLYPH,
&any, 0, 0, iflags.menu_headings, buf,
MENU_UNSELECTED);
}
fake_hero_object = zeroobj;
fake_hero_object.
quan = 1L; /* not strictly necessary... */
any.
a_obj = &fake_hero_object;
add_menu(win, mon_to_glyph(
&youmonst, rn2_on_display_rng), &any,
/* fake inventory letter, no group accelerator */
CONTAINED_SYM, 0, ATR_NONE,
        ...
        struct obj *item;
{
    ...
    if (*u.ushops && (shkp = shop_keeper(*u.ushops)) != 0) {
        ...
        loss = stolen_value(item, u.ux, u.uy, (boolean)
        shkp->mpeaceful,
                TRUE);
    }
...
{
    int FDECL((
    *actionfunc), (OBJ_P)), FDECL((*checkfunc), (OBJ_P));
    struct obj **objlist;
