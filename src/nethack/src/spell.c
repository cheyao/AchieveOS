...
STATIC_DCL boolean
FDECL(spell_aim_step, (genericptr_t,
int, int));
...
&& !(
IS_DOOR(levl[x][y]
.typ) && (levl[x][y].doormask & D_ISOPEN)))
return
FALSE;
return
TRUE;
...
throwspell() {
    ...
    walk_path(&uc, &cc, spell_aim_step, (genericptr_t)
    0);
...
sortspells() {
    ...
    qsort((genericptr_t)
    spl_orderindx, n, sizeof *spl_orderindx, spell_cmp);
    return;
}
