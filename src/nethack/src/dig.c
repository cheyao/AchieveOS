        ...
        struct obj *obj;
{
    ...
    for (sdp = Cmd.dirchars; *sdp; ++sdp) {
        ...
        if (u.uswallow) { ; /* all directions are viable when swallowed */
        } else if (movecmd(*sdp)) {
        ...
        boolean *dealloced;
{
     ...
     * are already handled.
     */
    if (otmp->otyp == CORPSE) { ; /* should cancel timer if under_ice */
        ...
        int x, y;
{
    ...
    for (otmp = level.objects[x][y]; otmp; otmp = otmp2) {
        if (costly) {
            loss += stolen_value(otmp, x, y, (boolean)
            shkp->mpeaceful, TRUE);
            if (otmp->oclass != COIN_CLASS)
                otmp->no_charge = 1;
        }
        otmp2 = bury_an_obj(otmp, (boolean * )
        0);
    }
