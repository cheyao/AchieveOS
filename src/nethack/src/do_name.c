...
*getpos_hilitefunc), (int)) = (

void FDECL((

*), (int))) 0;

static boolean FDECL((

*getpos_getvalid), (int, int)) =
(
        ...
        const char *goal;
{
    ...
    for (i = 0; i < NUM_GLOCS; i++)
        if (garr[i])
            free((genericptr_t) garr[i]);
    getpos_hilitefunc = (void FDECL((
    *), (int))) 0;
    getpos_getvalid = (boolean
    FDECL((*),(
    int, int))) 0;
    return result;
}
