...
*select_iter_func), (int, int, genericptr));
...
STATIC_DCL boolean
FDECL(is_ok_location, (SCHAR_P, SCHAR_P,
int));
STATIC_DCL unpacked_coord
...
STATIC_DCL boolean
FDECL(sp_level_loader, (dlb
*, sp_lev *));
STATIC_DCL boolean
FDECL(sp_level_free, (sp_lev
*));
...
STATIC_VAR int FDECL((

*selection_flood_check_func), (int, int));
STATIC_VAR schar
floodfillchk_match_under_typ;
...
#define SEL_FLOOD_CHKDIR(mx, my, sel) \
    ...
    if (selection_flood_check_func == (int FDECL((
    *), (int, int))) 0) {
        opvar_free(tmp);
        return;
    }
