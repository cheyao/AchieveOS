...
n = 0, o = *olist;
o;
o = by_nexthere ? o->nexthere : o->nobj
)
++
n;
...
mode &= ~
SORTLOOT_PETRIFY; /* remove flag, leaving mode */
...
str = (char *) 0;
sliarray[i].
orderclass = sliarray[i].subclass = sliarray[i].disco = 0;
++
i;
}
n = i;
...
sliarray[n].
obj = (struct obj *) 0, sliarray[n]
.
indx = -1;
sliarray[n].
str = (char *) 0;
sliarray[n].
orderclass = sliarray[n].subclass = sliarray[n].disco = 0;
...
qsort((genericptr_t)
sliarray, n, sizeof *sliarray, sortloot_cmp);
...
ilets[iletct++] = 'u';
if (
count_buc(invent, BUC_BLESSED, ofilter
))
ilets[iletct++] = 'B';
if (
count_buc(invent, BUC_UNCURSED, ofilter
))
ilets[iletct++] = 'U';
if (
count_buc(invent, BUC_CURSED, ofilter
))
ilets[iletct++] = 'C';
if (
count_buc(invent, BUC_UNKNOWN, ofilter
))
ilets[iletct++] = 'X';
ilets[iletct++] = 'a';
}
...
->invlet))
(void)
strkitten(ailets, otmp
->invlet);
if (
display_inventory(ailets, TRUE
) == '\033')
return 0;
} else
break;
...
if (uwep)
(void)
strkitten(extra_removeables, uwep
->oclass);
if (uswapwep)
(void)
strkitten(extra_removeables, uswapwep
->oclass);
if (uquiver)
(void)
strkitten(extra_removeables, uquiver
->oclass);
}
...
if (
index(extra_removeables, oc_of_sym
)) { ; /* skip rest of takeoff checks */
} else if (!
index(removeables, oc_of_sym
)) {
pline("Not applicable.");
return 0;
} else if (oc_of_sym == ARMOR_CLASS && !
...
allflag = TRUE;
} else if (sym == 'A') { ; /* same as the default */
} else if (sym == 'u') {
...
while ((
otmp = nxt_unbypassed_loot(sortedchn, *objchn)
) != 0) {
if (ilet == 'z')
ilet = 'A';
else if (ilet == 'Z')
...
STATIC_VAR NEARDATA
const char oth_symbols[] = {CONTAINED_SYM, '\0'};
STATIC_VAR NEARDATA
const char *oth_names[] = {"Bagged/Boxed items"};

STATIC_VAR NEARDATA
char *invbuf = (char *) 0;
STATIC_VAR NEARDATA
unsigned invbufsiz = 0;
