...
STATIC_DCL boolean
FDECL(inherits,
(struct monst *, int, int, BOOLEAN_P));
        ...
        register struct monst *shkp;
{
    ...
    if (credit == 0L) { ; /* nothing to do; just 'return tmp;' */
    } else if (credit >= tmp) {
...
UNUSED; /* <ox,oy> predate 'noit_Monnam()', let alone Shknam() */
xchar oy
UNUSED;
{
struct eshk *eshkp = ESHK(shkp);
        ...
        boolean croaked;
{
    ...
    if (skip_msg) { ; /* already gave an untrap message which covered the only repair */
    } else if (saw_walls) {
        char wallbuf[BUFSZ];
        ...
        register int fall;
{
    ...
    lang = 0;
    if (shkp->msleeping || !shkp->mcanmove || is_silent(shkp->data)); /* lang stays 0 */
