...
dodown() {
    ...
    if (HLevitation || ELevitation) {
        ...
        if (BLevitation) { ; /* weren't actually floating after all */
        } else if (Blind) {
    ...
    if (portal && !In_endgame(&u.uz)) {
        ...
        if (up) {
            ...
            if (at_ladder)
                ...
                u_on_upstairs();
            if (!u.dz) { ; /* stayed on same level? (no transit effects) */
            } else if (Flying) {
                if (flags.verbose)
                    You("fly down %s.",
