        ...
        unsigned doname_flags;
{
    ...
    if (iflags.suppress_price || restoring) { ; /* don't attempt to obtain any stop pricing, even if 'with_price' */
        ...
        unsigned cxn_flags; /* bitmask of CXN_xxx values */
{
    ...
    *nambuf = '\0';
    ...
    if (glob) { ; /* omit_corpse doesn't apply; quantity is always 1 */
    } else if (!omit_corpse) {
...
*func), (OBJ_P)),    /* main formatting routine */
*
FDECL((*altfunc), (OBJ_P)
...
outbuf = (*func)(obj);
objects[obj->otyp].
oc_uname = save_uname; /* restore called string */
if ((unsigned)
strlen(outbuf)
<= lenlimit)
return
...
save_obj = *obj;
obj->
bknown = obj->rknown = obj->greased = 0;
obj->
oeroded = obj->oeroded2 = 0;
releaseobuf(outbuf);
outbuf = (*func)(obj);
if (altfunc && (unsigned)
strlen(outbuf)
    ...
    *outbuf = '\0';
    ...
    if (!str[1]) {
        ...
        Strcpy(outbuf, index("aefhilmnosx", c0) ? "an " : "a ");
               ...
               || !strcmpi(str, "iron bars") || !strcmpi(str, "ice")) { ; /* no article */
    } else {
...
#define SPINACH 2
    ...
    for (;;) {
        ...
        if (!strncmpi(bp, "an ", l = 3) || !strncmpi(bp, "a ", l = 2)) {
            cnt = 1;
        } else if (!strncmpi(bp, "the ", l = 4)) { ; /* just increment `bp' by `l' below */
    ...
    if (!BSTRCMPI(bp, p - 6, " stone") || !BSTRCMPI(bp, p - 4, " gem")) {
        ...
        dn = actualn = bp;
        goto srch;
    } else if (!strcmpi(bp, "looking glass")) { ; /* avoid false hit on "* glass" */
...
len +=
len_qsfx; /* include the pending suffix */
/* format the object */
