...
STATIC_DCL boolean
FDECL(emergency_disrobe, (boolean
*));
...
STATIC_DCL boolean
FDECL(thitm,
(int, struct monst *, struct obj *, int,
BOOLEAN_P));
        ...
        unsigned ftflags;
{
    ...
    if (Sokoban && Can_fall_thru(&u.uz)); /* KMH -- You can't escape the Sokoban level traps */
        ...
        int *fail_reason;
{
    ...
    if (!context.mon_moving) {
        ...
        if (cause != ANIMATE_NORMAL && costly_spot(x, y)
            ...
            && mon != shkp)
            (void) stolen_value(statue, x, y, (boolean)
        shkp->mpeaceful,
                FALSE);
    ...
    if (Luck > -13 && rn2(13 + Luck) > 7) { /* saved by luck */
        ...
        switch (rn2(20) ? ((Luck >= 13) ? 0 : rn2(13 - Luck)) : rn2(26)) {
            ...
            case 21: {
                ...
                if (costly)
                    loss += stolen_value(obj, ox, oy, (boolean)
                shkp->mpeaceful,
                        TRUE);
                delete_contents(obj);
...
sink_into_lava() {
    ...
    if (!u.utrap || u.utraptype != TT_LAVA) { ; /* do nothing; this usually won't happen but could after
