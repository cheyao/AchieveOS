        ...
        boolean by_hero;
{
    ...
    if (by_hero) {
        ...
        if (shkp && mtmp != shkp)
            (void) stolen_value(corpse, x, y, (boolean)
        shkp->mpeaceful,
                FALSE);
...
if (poly_zapped >= 0)
create_polymon(level
.objects[tx][ty], poly_zapped);
    ...
    switch (obj->otyp) {
        ...
        case SPE_WIZARD_LOCK:
            ...
            if (((levl[x][y].typ == DRAWBRIDGE_DOWN)
                ...
                if (!striking && closeholdingtrap(&youmonst, &disclose)) { ; /* now stuck in web or bear trap */
        ...
        int dmg; /* base amount to be adjusted by bonus or penalty */
{
    ...
    if (intell <= 9) {
        ...
        if (dmg > 1)
            dmg = (dmg <= 3) ? 1 : dmg - 3;
    } else if (intell <= 13 || u.ulevel < 5); /* no bonus or penalty; dmg remains same */
    else if (intell <= 18)
        dmg += 1;
...
*fhitm), (MONST_P, OBJ_P)), /* fns called when mon/obj hit */
...
show_transient_light(obj, bhitpos
.x, bhitpos.y);
}
}
...
&&
glyph_is_invisible(levl[bhitpos.x][bhitpos.y]
.glyph)) {
unmap_object(bhitpos
.x, bhitpos.y);
newsym(x, y
);
}
...
0);
goto
bhit_done; /* result == (struct monst *) 0 */
}
}
...
&& *
in_rooms(bhitpos
.x, bhitpos.y, SHOPBASE)) {
shopdoor = TRUE;
add_damage(bhitpos
.x, bhitpos.y, SHOP_DOOR_COST);
}
}
break;
}
...
context.
mon_moving = TRUE; /* hero isn't causing this ice to melt */
