...
Sprintf(tmpbuf,
"%s ", genders[flags.female ? 1 : 0].adj);
Sprintf(buf,
"%sin %s%s form", !final ? "currently " : "", tmpbuf,
uasmon->mname);
you_are(buf, "");
}
...
Sprintf(tmpbuf,
"%s ", genders[innategend].adj);
buf[0] = '\0';
if (Upolyd)
Strcpy(buf,
"actually "); /* "You are actually a ..." */
if (!
strcmpi(rank_titl, role_titl
)) {
...
Sprintf(buf,
"the dungeon %ld turn%s ago", moves,
plur(moves)
);
...
if (final ? iflags.at_midnight :

midnight()

) {
enl_msg("It ", "is ", "was ", "the midnight hour", "");
} else if (final ? iflags.at_night :
...
Sprintf(buf,
" Bad things %s on Friday the 13th.",
!final ? "can happen"
...
if (hp == hpmax && hpmax > 1)
Sprintf(buf,
"all %d hit points", hpmax);
else
Sprintf(buf,
"%d out of %d hit point%s", hp, hpmax,
plur(hpmax)
);
you_have(buf, "");
...
Sprintf(buf,
"%s %s", !pwmax ? "no" : "both", Power);
else if (pw == pwmax && pwmax > 2)
Sprintf(buf,
"all %d %s", pwmax, Power);
else
Sprintf(buf,
"%d out of %d %s", pw, pwmax, Power);
you_have(buf, "");
...
default:
Sprintf(buf,
"%d hit dice", mons[u.umonnum].mlevel);
break;
}
you_have(buf, "");
}
...
Sprintf(buf,
"aware of the presence of %s",
makeplural(mons[context.warntype.speciesidx]
.mname));
you_are(buf, from_what(WARN_OF_MON));
}
if (Undead_warning)
...
if (
lays_eggs(youmonst
.data) && flags.female) /* Upolyd */
you_can("lay eggs", "");
if (u.ulycn >= LOW_PM) {
/* "you are a werecreature [in beast form]" */
Strcpy(buf, an(mons[u.ulycn]
.mname));
        ...
        int msgflag;          /* for variant message phrasing */
{
    ...
    } else { ; /* shouldn't happen; will result in generic "you are hiding" */
    }
        ...
        register char *cmd;
{
    ...
    if (prefix_seen && cmd[0] == Cmd.spkeys[NHKF_REQMENU]) {
        ...
        int NDECL((
        *func)) = ft ? ((struct ext_func_tab *) ft)->ef_funct : 0;
...
any.
a_nfunc = func;
add_menu(win, NO_GLYPH,
&any, 0, 0, ATR_NONE, text, MENU_UNSELECTED);
}
}
