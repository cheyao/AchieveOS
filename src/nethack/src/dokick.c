        ...
        xchar x, y;
{
    ...
    if (!uarmf && kickedobj->otyp == CORPSE
        ...
        if (poly_when_stoned(youmonst.data) &&
            polymon(PM_STONE_GOLEM)) { ; /* hero has been transformed but kick continues */
        } else {
    ...
    *), (OBJ_P, OBJ_P))) 0, &kickedobj);
    ...
    if (costly && (!costly_spot(bhitpos.x, bhitpos.y)
        ...
        if (isgold)
            ...
            (void) stolen_value(kickedobj, x, y, (boolean)
        shkp->mpeaceful,
                FALSE);
    }
