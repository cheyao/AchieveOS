...
STATIC_PTR boolean
FDECL(check_jump, (genericptr_t,
int, int));
STATIC_DCL boolean
FDECL(is_valid_jump_pos,
(int, int, int, BOOLEAN_P));
STATIC_DCL boolean
FDECL(get_valid_jump_position,
(int, int));
STATIC_DCL boolean
FDECL(get_valid_polearm_position,
(int, int));
STATIC_DCL boolean
FDECL(find_poleable_mon, (coord
*, int, int));
        ...
        int rx, ry, *resp;
{
    ...
    if (!corpse && !statue) { ; /* nothing to do */
        ...
        register xchar x, y;
{
    ...
    for (otmp = invent; otmp; otmp = otmp->nobj) {
        ...
        if (dist2(u.ux, u.uy, mtmp->mx, mtmp->my)
            ...
            if (!um_dist(mtmp->mx, mtmp->my, 3)) { ; /* still close enough */
        ...
        struct obj *obj;
{
    ...
    if ((mtmp = m_at(bhitpos.x, bhitpos.y)) != (struct monst *) 0) {
        ...
        if (t && t->ttyp == STATUE_TRAP
            && activate_statue_trap(t, t->tx, t->ty, FALSE)) { ; /* feedback has been give by animate_statue() */
        } else {
