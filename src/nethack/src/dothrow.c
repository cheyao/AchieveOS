...
STATIC_DCL boolean
FDECL(mhurtle_step, (genericptr_t,
int, int));
...
autoquiver() {
    ...
    for (otmp = invent; otmp; otmp = otmp->nobj) {
        if (otmp->owornmask || otmp->oartifact || !otmp->dknown) { ; /* Skip it */
            ...
            if (uslinging())
                ...
                omisc = otmp;
        } else if (otmp->oclass == GEM_CLASS) { ; /* skip non-rock gems--they're ammo but
...
*check_proc), (genericptr_t, int, int));
genericptr_t arg;
{
        ...
        int x, y;
{
     ...
     */
    if ((ttmp = t_at(x, y)) != 0) {
        if (stopping_short) { ; /* see the comment above hurtle_jump() */
        ...
        int dx, dy, range;
{
    ...
    (void) walk_path(&mc, &cc, mhurtle_step, (genericptr_t)
    mon);
    return;
}
        ...
        boolean broken;
{
    ...
    if (broken || !costly_xy || *in_rooms(x, y, SHOPBASE) != *u.ushops) {
        ...
        if (is_unpaid(obj))
            (void) stolen_value(obj, u.ux, u.uy, (boolean)
        shkp->mpeaceful,
                FALSE);
        if (broken)
            obj->no_charge = 1;
        ...
        boolean twoweap; /* used to restore twoweapon mode if wielded weapon returns */
{
    ...
    throwit_return:
    iflags.returning_missile = (genericptr_t)
    0;
    if (clear_thrownobj)
        thrownobj = (struct obj *) 0;
    return;
