...
void set_idt(u32int vector, u32int offset) {
    idt[vector].low         = (u16int)(((offset) >> 16) & 0xFFFF);   // (u16int)(((offset) >> 16) & 0xFFFF)
    idt[vector].selector    = 0x08;             // Sector
    idt[vector].zero        = 0;                // Reserved
    idt[vector].type        = 0x8e;             // ISR = 0x8E
    idt[vector].high        = (u16int)((offset) & 0xFFFF);  //
}
...
void init_idt() {
    ...
    idtr.base  = (u32int) &idt[0];
    idtr.limit = sizeof(idt_t) * 48 - 1;
