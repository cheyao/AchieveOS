...
(struct dungeon *));
STATIC_DCL boolean
FDECL(unreachable_level, (d_level
*, BOOLEAN_P));
...
STATIC_DCL boolean
FDECL(interest_mapseen, (mapseen
*));
...
init_dungeons() {
    ...
    for (i = 0; i < n_dgns; i++) {
        ...
        if (pd.tmpdungeon[i].lev.rand)
            dungeons[i].num_dunlevs = (xchar)
        rn1(pd.tmpdungeon[i].lev.rand,
...
struct monst *priest
UNUSED; /* currently unused; might be useful someday */
{
mapseen *mptr = find_mapseen(&u.uz);
